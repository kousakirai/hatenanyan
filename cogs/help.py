...
class JapaneseHelpCommand(commands.DefaultHelpCommand):
  ...
  async def send_group_help(self,group):
    embed = discord.Embed(title=self.get_command_signature(group),description=group.description,color=0x00ff00)
    if group.help:
    ...
    for command in command_list:
        content += f"`{command.name}` / {command.description}\n"
    embed.add_field(name="サブコマンドリスト",value=content,inline=False)
    embed.set_footer(text=f"コマンドのヘルプ {self.context.prefix}help コマンド名")
    await self.get_destination().send(embed=embed)
  async def send_command_help(self,command):
    embed = discord.Embed(title=self.get_command_signature(command),description=command.description,color=0x00ff00)
    if command.help:
